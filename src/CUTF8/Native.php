<?php
namespace Cresenity\Laravel\CUTF8;

use Cresenity\Laravel\CUTF8;
use Cresenity\Laravel\CUTF8\Traits\LowerUpperTrait;
use Cresenity\Laravel\CUTF8\Exception as CUTF8Exception;
...
class Native {
    ...
    public static function ord($chr) {
        ...
        if (!isset($chr[1])) {
            throw new CUTF8Exception('Short sequence - at least 2 bytes expected, only 1 seen');
        }
        ...
        if (!isset($chr[2])) {
            throw new CUTF8Exception('Short sequence - at least 3 bytes expected, only 2 seen');
        }
        ...
        if (!isset($chr[3])) {
            throw new CUTF8Exception('Short sequence - at least 4 bytes expected, only 3 seen');
        }
        ...
        if (!isset($chr[4])) {
            throw new CUTF8Exception('Short sequence - at least 5 bytes expected, only 4 seen');
        }
        ...
        if (!isset($chr[5])) {
            throw new CUTF8Exception('Short sequence - at least 6 bytes expected, only 5 seen');
        }
        ...
        if ($ord0 >= 254 and $ord0 <= 255) {
            throw new CUTF8Exception("Invalid UTF-8 with surrogate ordinal ':ordinal'", [
                ':ordinal' => $ord0,
            ]);
        }
    }
    ...
    public static function fromUnicode($arr) {
        ...
        foreach ($keys as $k) {
            ...
            if (($arr[$k] >= 0) and ($arr[$k] <= 0x007f)) {
                ...
                // Found a surrogate
                throw new CUTF8Exception("CUTF8::from_unicode: Illegal surrogate at index: ':index', value: ':value'", [
                    ':index' => $k,
                    ':value' => $arr[$k],
                ]);
            } elseif ($arr[$k] <= 0xffff) {
                ...
                // Out of range
                throw new CUTF8Exception("CUTF8::from_unicode: Codepoint out of Unicode range at index: ':index', value: ':value'", [
                    ':index' => $k,
                    ':value' => $arr[$k],
                ]);
            }
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            if ($m_state == 0) {
                ...
                if (0x80 == (0xC0 & $in)) {
                    ...
                    throw new CUTF8Exception("CUTF8::to_unicode: Incomplete multi-octet sequence in UTF-8 at byte ':byte'", [
                        ':byte' => $i,
                    ]);
                }
            }
    ...
    public static function strPad($str, $final_str_length, $pad_str = ' ', $pad_type = STR_PAD_RIGHT) {
        ...
        throw new CUTF8Exception('CUTF8::str_pad: Unknown padding type (:pad_type)', [
            ':pad_type' => $pad_type,
        ]);
    }
