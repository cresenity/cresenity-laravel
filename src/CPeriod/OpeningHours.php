...
use Cresenity\Laravel\CPeriod\Exception\InvalidDateTimeClass;
use Cresenity\Laravel\CPeriod\Exception\InvalidTimezoneException;
use Cresenity\Laravel\CPeriod\OpeningHours\Day;
use Cresenity\Laravel\CPeriod\OpeningHours\Exception\InvalidDayNameException;
use Cresenity\Laravel\CPeriod\OpeningHours\Exception\MaximumLimitExceededException;
use Cresenity\Laravel\CPeriod\OpeningHours\Helper;
use Cresenity\Laravel\CPeriod\OpeningHours\OpeningHoursForDay;
use Cresenity\Laravel\CPeriod\OpeningHours\PreciseTime;
use Cresenity\Laravel\CPeriod\OpeningHours\Time;
use Cresenity\Laravel\CPeriod\OpeningHours\TimeRange;
use Cresenity\Laravel\CPeriod\OpeningHours\Traits\DiffTrait;
...
class OpeningHours
{
    ...
    /**
     * @var \Cresenity\Laravel\CPeriod\OpeningHours\OpeningHoursForDay[]
     */
     ...
     *
     * @return \Cresenity\Laravel\CPeriod\OpeningHours\OpeningHoursForDay
     */
    ...
    public function nextOpen(DateTimeInterface $dateTime = null): DateTimeInterface
    {
        ...
        while ($nextOpen === false || $nextOpen->hours() >= 24) {
            if (--$tries < 0) {
                throw MaximumLimitExceededException::forString(
    ...
    public function previousClose(DateTimeInterface $dateTime)
    {
        ...
        $tries = $this->getDayLimit();
        /** @var \Cresenity\Laravel\CPeriod\OpeningHours\Time $previousClose */
    ...
    {
        return \carr::map($this->regularClosingDays(), [Day::class, 'toISO']);
    }
        ...
        $this->exceptions = \carr::map($exceptions, static function (array $openingHours, string $date) {
            ...
            if ($recurring === false || $recurring->format('m-d') !== $date) {
                ...
                if ($dateTime === false || $dateTime->format('Y-m-d') !== $date) {
                    throw InvalidDateException::forFormat($date, 'Y-m-d');
                }
            }
    ...
    protected function normalizeDayName($day)
    {
        ...
        if (!Day::isValid($day)) {
            throw InvalidDayNameException::invalidDayName($day);
        }
    ...
    {
        return Helper::flatMap($this->openingHours, $callback);
    }
    ...
    {
        return Helper::flatMap($this->exceptions, $callback);
    }
    ...
    {
        $regularHours = $this->flatMap(static function (OpeningHoursForDay $openingHoursForDay, $day) use ($format, $timezone) {
        ...
        $exceptions = $this->flatMapExceptions(static function (OpeningHoursForDay $openingHoursForDay, $date) use ($format, $timezone) {
    ...
    private function parseTimezone($timezone)
    {
        ...
        if ($timezone) {
            throw InvalidTimezoneException::create();
        }
