...
use Cresenity\Laravel\CManager\Transform\Method\ClosureMethod;
...
class Parser
{
    ...
    protected static function prepareMethod($method)
    {
        if ($method instanceof Closure || $method instanceof \Opis\Closure\SerializableClosure) {
            $method = new ClosureMethod($method);
        }

        if (!is_object($method)
            || $method instanceof TransformMethodInterface
        ) {
            return $method;
        }
    ...
    {
        if ($data instanceof Model || $data instanceof Arrayable) {
            $data = $data->toArray();
        }
